...
// in the editor so the user sees something instead of a broken image icon.
const HERO_PLACEHOLDER = 'https://placehold.co/600x300/f0ede9/9ca3af?text=Click+to+replace+hero+image'

function prepareHtmlForEditor(html: string): string {
  return html.replace(/\{\{HERO_IMAGE\}\}/g, HERO_PLACEHOLDER)
}

/* eslint-disable @typescript-eslint/no-explicit-any */
function getEditor(): any { return (window as any).tinymce?.get(EDITOR_ID) }
/* eslint-enable @typescript-eslint/no-explicit-any */

/* ══════════════════════════════════════════════════════════════════
   IMAGE UPLOADER SUBCOMPONENT
   ══════════════════════════════════════════════════════════════════ */
            ...
            <div key={a.previewUrl} className="relative rounded-lg overflow-hidden bg-gray-100 aspect-square group">
              ...
              {!a.uploading && !a.error && (
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100">
                  {onInsert && a.url && <button type="button" onClick={(e) => { e.stopPropagation(); onInsert(a.url) }}
...
export function MarketingStudioClient({ audiences, mailchimpServer = 'us1' }: Props) {
  ...
  const [html, setHtml] = useState('')               // for preview iframe only
  ...
  /* ═══════════════════════════════════════════════════════════════
     REFS — The fix for stale closures.
     TinyMCE callbacks capture the closure from the render they were
     created in. By reading from refs instead of state, we always
     get the latest value even inside stale closures.
     ═══════════════════════════════════════════════════════════════ */
  ...
  const isLoadingRef = useRef(false)       // true during template switch
  ...
  // Ref for templatesByCategory so switchCategory always reads the latest
  const tplByCatRef = useRef(templatesByCategory)
  tplByCatRef.current = templatesByCategory
  ...
  /* ═══════════════════════════════════════════════════════════════
     AUTO-SAVE — Reads from REFS, not state. No stale closures.
     ═══════════════════════════════════════════════════════════════ */
  async function doSave(silent: boolean) {
    if (isLoadingRef.current) return
    const editor = getEditor()
    const currentHtml = editor?.getContent() || ''
    const ctx = saveDataRef.current
    const name = ctx.name || CATEGORIES.find((c) => c.key === ctx.category)?.label || 'Template'
    const subj = ctx.subject || '(No subject)'
    if (!name || !subj || !currentHtml) return
    ...
    try {
      ...
      setSaveStatus('saved')
      // If this was a new template, capture the generated ID
      if (data.template?.id && !ctx.templateId) {
        setTemplateId(data.template.id)
        saveDataRef.current.templateId = data.template.id
      }
      // Quietly refresh the templates list (for timestamps on category cards)
  ...
  // Stable ref so TinyMCE's stale closure always calls the latest version
  ...
  /* ═══════════════════════════════════════════════════════════════
     SWITCH CATEGORY — Atomic operation with loading guard.
     Cancels pending auto-save, sets guard, updates state + refs
     synchronously, sets TinyMCE content, then clears guard after
     TinyMCE's change event has passed.
     ═══════════════════════════════════════════════════════════════ */
  function switchCategory(cat: string) {
    // 1. Cancel any pending auto-save from the previous category
    if (autoSaveTimerRef.current) clearTimeout(autoSaveTimerRef.current)

    // 2. Set loading guard — TinyMCE change events will be ignored
    ...
    const tpl = tplByCatRef.current[cat]
    if (tpl) {
      ...
      // 4. Sync ref IMMEDIATELY (don't wait for React re-render)
      saveDataRef.current = {
        templateId: tpl.id,
        category: cat,
        name: tpl.name,
      ...
      // 5. Load into TinyMCE
      const editor = getEditor()
      if (editor) editor.setContent(prepareHtmlForEditor(tpl.html_content))
    } else {
      const catObj = CATEGORIES.find((c) => c.key === cat)
      setTemplateId(null)
      setTemplateName(catObj?.label || '')
      setSubject('')
      setPreheader('')
      setHtml('')
      saveDataRef.current = {
    ...
    lastLoadedCategoryRef.current = cat

    // 6. Clear guard AFTER TinyMCE's change event from setContent() has fired
    setTimeout(() => { isLoadingRef.current = false }, 300)
  }

  /* ── Load initial template after data arrives ───────────────── */
  useEffect(() => {
    if (loading || templates.length === 0) return
    if (lastLoadedCategoryRef.current !== null) return // already loaded once
    switchCategory(activeCategory)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, templates])

  /* ═══════════════════════════════════════════════════════════════
     INIT TinyMCE — ONCE. No dependency on `view`.
     The textarea is always in the DOM (hidden with CSS when not in
     editor view). TinyMCE attaches once and stays alive.
     ═══════════════════════════════════════════════════════════════ */
  useEffect(() => {
    if (!tinyLoaded || editorInitialized.current) return
    // Wait for editor view to be visible (TinyMCE needs dimensions)
    ...
    ;(window as any).tinymce?.init({
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      setup: (editor: any) => {
        // After TinyMCE is fully ready, load the current template
        editor.on('init', () => {
          const tpl = tplByCatRef.current[saveDataRef.current.category]
          if (tpl) {
            isLoadingRef.current = true
            editor.setContent(prepareHtmlForEditor(tpl.html_content))
            setTimeout(() => { isLoadingRef.current = false }, 300)
          }
        })
        editor.on('change keyup', () => {
          // KEY FIX: Ignore changes fired by programmatic setContent()
          if (isLoadingRef.current) return
          const content = editor.getContent()
          setHtml(content)
          // KEY FIX: Always call the LATEST version via ref
  ...
  /* ── Update meta fields — sync ref immediately ─────────────── */
  function updateMeta(field: 'name' | 'subject' | 'preheader', value: string) {
    switch (field) {
      case 'name': setTemplateName(value); break
      case 'subject': setSubject(value); break
      case 'preheader': setPreheader(value); break
    }
    // Sync the ref IMMEDIATELY so auto-save reads the new value
    saveDataRef.current = { ...saveDataRef.current, [field]: value }
    triggerAutoSaveRef.current()
  }
  ...
  function insertMergeTag(tag: string) {
    const editor = getEditor()
    if (!editor) return
    if (showPreview) setShowPreview(false)   // switch to edit mode
    editor.focus()                           // KEY FIX: ensure focus
  ...
  async function createCampaign() {
    const tpl = campaignTemplate ? tplByCatRef.current[campaignTemplate] : tplByCatRef.current[activeCategory]
  ...
  /* ══════════════════════════════════════════════════════════════
     RENDER — TinyMCE textarea is ALWAYS in the DOM.
     Views are shown/hidden with display, NOT conditional rendering.
     This prevents TinyMCE from being destroyed on view switch.
     ══════════════════════════════════════════════════════════════ */
  return (
    <>
      <Script
      ...
      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden min-h-[760px] flex flex-col">
        ...
        {/* ═══════════════════════════════════════════════════════
            EDITOR VIEW — shown/hidden with display, NOT unmounted
        ═══════════════════════════════════════════════════════ */}
        <div className="flex-1 flex flex-col overflow-hidden" style={{ display: view === 'editor' ? 'flex' : 'none' }}>
          ...
          <div className="flex-1 flex divide-x divide-gray-100 overflow-hidden">
            ...
            {/* LEFT sidebar: meta + merge tags + images + actions */}
            <div className="w-72 flex-shrink-0 flex flex-col bg-[#fafaf9] overflow-y-auto">
              ...
              <div className="p-4 border-b border-gray-100 space-y-3">
                ...
                <div>
                  ...
                  <input value={templateName} onChange={(e) => updateMeta('name', e.target.value)}
                ...
                <div>
                  ...
                  <input value={subject} onChange={(e) => updateMeta('subject', e.target.value)}
                ...
                <div>
                  ...
                  <input value={preheader} onChange={(e) => updateMeta('preheader', e.target.value)}
            ...
            <div className="flex-1 flex flex-col min-w-0">
              ...
              <div className="px-4 py-2 border-b border-gray-100 flex items-center justify-between bg-white">
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={() => { setHtml(getEditor()?.getContent() || html); setShowPreview(true) }}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${showPreview ? 'bg-[#03374f] text-white' : 'text-gray-500 hover:text-gray-700'}`}>
                    Preview
                  </button>
                </div>
              ...
              {/* TinyMCE — ALWAYS in DOM, hidden during preview with CSS */}
              <div className="flex-1 overflow-auto bg-[#f0ede9]" style={{ display: showPreview ? 'none' : 'block' }}>
              ...
              {/* Preview */}
              {showPreview && (
                <div className="flex-1 overflow-auto bg-[#f0ede9] p-6">
        ...
        {/* ═══════════════════════════════════════════════════════
            CAMPAIGN VIEW
        ═══════════════════════════════════════════════════════ */}
        <div className="flex-1 flex flex-col overflow-hidden" style={{ display: view === 'campaign' ? 'flex' : 'none' }}>
          {/* Template selection */}
          <div className="px-6 py-4 border-b border-gray-100 bg-[#fafaf9]">
        ...
        {/* ═══════════════════════════════════════════════════════
            HISTORY VIEW
        ═══════════════════════════════════════════════════════ */}
        <div className="flex-1 p-6 overflow-auto" style={{ display: view === 'history' ? 'block' : 'none' }}>
          {history.length === 0 ? (
